...
def filter_data_by_month_range(df, date_str, date_column='Дата операции'):
    """
            ...
            for code in user_currencies:
                if code in valutes:
                    currencies_result.append({
                        "currency": code,
                        "rate": round(valutes[code]["Value"], 2)
                    })
